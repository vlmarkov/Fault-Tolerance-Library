...
/*****************************************************************************/
void grid_task_init(grid_task_t *grid, grid_task_e mode)
{
    int counter = 0;
                                   ...
                                   const int    col)
{
    ...
    {
        ri = check_overflow(ri, grid->cols_per_proc);
        ...
        for (int y = 0; y < y_times; y++)
        {
            ...
            if (grid->mode == COMPUTE_REDUNDANCY)
            {
                 ...
                 * Do not include self
                 */
                if (grid->tasks[ri][rj].real_task[0]->rank != rank)
                {
                    grid->tasks[ri][rj].real_task[idx] = grid_task_get(grid, rank);
                    grid->tasks[ri][rj].real_counter  += 1;
                }
            }
